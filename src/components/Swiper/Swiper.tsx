...
// Միայն կառուցվածքային տվյալները (տեքստերը գտնվում են i18n-ի մեջ)
...
const HeroSlider: React.FC = () => {
  ...
  return (
    <section className="relative w-full bg-[#e5e5e5] overflow-hidden">
      ...
      {/* Ակտիվ կետիկի (Active Bullet) գույնը սահմանող CSS ոճ */}
      <style>{`
      ...
      >
        {slidesData.map((slide, index) => {
          // Տեքստերի ստացում i18n-ից ըստ ընթացիկ սլայդի ինդեքսի
          const title = t(`heroSlider.${index}.title`);
          const description = t(`heroSlider.${index}.description`);
          const buttonText = t(`heroSlider.${index}.buttonText`);
